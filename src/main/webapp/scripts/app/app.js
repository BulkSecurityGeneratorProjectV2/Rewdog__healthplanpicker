...
angular.module(
		'healthPlanPickerApp',
		[ 'LocalStorageModule', 'ngResource', 'ngCookies', 'ngAria',
				'ngCacheBuster', 'ngFileUpload',
...
.run(
		function($rootScope, $location, $window, $http, $state, Auth,
			...
			$rootScope.$on('$stateChangeSuccess', function(event, toState,
				...
				var titleKey = 'HealthPlanPicker';

				// Remember previous state unless we've been redirected to login
				// or we've just
				// reset the state memory after logout. If we're redirected to
				// login, our
				// previousState is already set in the authExpiredInterceptor.
				// If we're going
...
// jhipster-needle-angularjs-add-config JHipster will add new application
// configuration
...
// This filter makes the assumption that the input will be in decimal form (i.e.
// 17% is 0.17).
